...
module Utils =
    ...
    let flatMap func input = 
        Option.flatten (Option.map func input)

    let getElement (lst:'a list) index =
        let rec toElement lst currentIndex =
            let checkIndex head tail =
                if index = currentIndex then Some(head)
                else toElement tail (currentIndex+1)
            nonEmptyListAction checkIndex lst
        if lst.IsEmpty then None
        else toElement lst 0
    ...
    let replaceElement (lst:'a list) (index:int) (newElement:'a) :'a list option =
        let rec iterateElements currentIndex headLst currentElement tailLst =
            if index = currentIndex then Some(headLst @ [newElement] @ tailLst)
...
module IntcodeInterpreter =
    ...
    let start (input: int list) =
        nextAction 0 input

    let setVerb (verb: int) memory =
        replaceElement memory 2 verb
    
    let setNoun (verb: int) memory =
        replaceElement memory 1 verb
